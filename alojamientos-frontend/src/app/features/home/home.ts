...
export class HomeComponent implements OnInit {
  ...
  cargarAlojamientos(): void {
    ...
    if (this.usandoBusquedaAvanzada && checkIn && checkOut) {
      ...
      // búsqueda simple solo por ciudad
      this.alojamientosService.listar(ciudadFiltro, this.page, this.size).subscribe({
        next: (resp: any) => {
          this.alojamientos = resp.content;
          this.total = resp.totalElements;
          this.loading = false;
  ...
  buscar(): void {
    ...
    // validación: si hay dos fechas, checkOut debe ser > checkIn
    if (checkIn && checkOut && checkOut <= checkIn) {
      this.errorBusqueda = 'La fecha de salida debe ser posterior a la de entrada.';
      return;
    }

    // si el usuario puso ambas fechas, usamos /disponibles
